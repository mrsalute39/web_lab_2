...
<div>
    <h3> ДАННЫЕ КВИЗА </h3>
    <div">
        <p><strong> Имя:</strong> ${username || '<span>Не указано</span>'}</p>
        <p><strong> Возраст:</strong> ${userage || '<span> Не указан</span>'}</p>
        <p><strong> Тема:</strong> ${themeText}</p>
        <p><strong> Приз:</strong> <span>${prize_bool}</span></p>
        <p><strong> Сложность:</strong> ${difficulty}</p>
    </div>
</div>
<hr">
        `;
        ...
        alert(message);
		 ...
		 resultDiv.innerHTML = htmlMessage + resultDiv.innerHTML;
    });
});